...
module.exports = class Warps {
  ...
  async init() {
    ...
    Omegga.on('cmd:setwarp', async (name, ...args) => {
      ...
      try {
        ...
        if (old) old.pos = pos;
        else this.warps.push({ name: dest, pos });
        await this.store.set('warps', this.warps);

        Omegga.whisper(
          player,
          `"Warp ${yellow(dest)} set. Use ${yellow('/warp ' + dest)} to warp."`
        );
      } catch (e) {
      ...
      .on('cmd:delwarp', async (name, ...args) => {
        ...
        if (warp === -1)
          return Omegga.whisper(
            name,
            `"A warp by that name does not exist. See available warps with ${yellow(
              '/warps'
            )}."`
          );
      ...
      .on('cmd:clearwarps', async name => {
        ...
        if (this.clearWarpsCooldown + 5000 > Date.now()) {
          ...
          Omegga.whisper(
            name,
            `"Send ${yellow('/clearwarps')} again within ${yellow(
              '5 seconds'
            )} to confirm. This will clear all warps and homes."`
          );
        }
      })
      .on('cmd:warp', (name, ...args) => {
        ...
        if (
          !auth &&
          (!isWarpEnabled || !this.cooldown(name, 'warp', warpCooldown))
        )
          return;
        if (args.length === 0)
          return Omegga.whisper(
            name,
            `"See available warps with ${yellow('/warps')}."`
          );

        const dest = Warps.cleanName(args);
        const warp =
          this.warps.find(w => w.name === dest) ||
          this.warps.find(w => w.name.toLowerCase() === dest.toLowerCase());
        if (!warp)
          return Omegga.whisper(
            name,
            `"A warp by that name does not exist. See available warps with ${yellow(
      ...
      .on('cmd:warps', async (name, arg) => {
        ...
        if (
          !auth &&
          (!isWarpEnabled || !this.cooldown(name, 'page', WARPS_PAGE_COOLDOWN))
        )
          return;
        ...
        if (this.warps.length !== 0) {
          ...
          for (
            let i = 0;
            i < PAGE_SIZE && i + offset < this.warps.length;
            i++
          ) {
            ...
            Omegga.whisper(
              name,
              `"- ${yellow(warp.name)} ${
                pos ? grey(`[${Math.round(dist(pos, warp.pos) / 10)}st]`) : ''
              }"`
            );
          }
      ...
      .on('cmd:home', async (name, ...args) => {
        ...
        if (
          !auth &&
          (!isHomeEnabled || !this.cooldown(name, 'warp', warpCooldown))
        )
          return;
        ...
        if (auth && args.length > 0) {
          ...
          if (!player) {
            return Omegga.whisper(
              player,
              '"Could not find an online player by that name."'
            );
          }
        } else {
        ...
        const home = (this.homes[id] =
          typeof this.homes[id] === 'undefined'
      ...
      .on('cmd:sethome', async name => {
        ...
        if (
          !auth &&
          (!isHomeEnabled || !this.cooldown(name, 'setHome', SET_HOME_COOLDOWN))
        )
          return;
        const player = Omegga.getPlayer(name);
        try {
          ...
          Omegga.whisper(
            player,
            `"Home warp has been set. Use ${yellow('/home')} to teleport home."`
          );
        } catch (e) {
          // player was not found... who cares
        }
